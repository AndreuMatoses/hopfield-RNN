...
%% initial pattern
n_cells=8;
accuracy_table=zeros(n_patterns,n_cells);
iter_table=accuracy_table; 
episodes=2000;
for n_pat=1:n_patterns
    for n_cell=1:n_cells
        for episode=1:episodes
            ...
            if all(pat_evolution(:,:,end)==pattern(:,:,n_pat))
                accuracy_table(n_pat,n_cell)=accuracy_table(n_pat,n_cell)+1/episodes;
                iter_table(n_pat,n_cell)=iter_table(n_pat,n_cell)+iter;
            end
        end
